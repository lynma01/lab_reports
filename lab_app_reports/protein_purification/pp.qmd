...
Measure the protein concentration of the final samples via A280 and Specific Activity assays. Repeat the SDS-PAGE to confirm EcoR1 presence in final sample.
...
- Wells #3, #4 are the only wells which contained *faint* banding. However, the cleavage in Wells #3, #4 are distant to the cleavage/distance within Well #1.
...
### Calculating Protein Purity of the Sample {#sec-r-calc-sa}
...
av=20 # Assay Volume for Total Units (mL)
pv=2000 # Pooled Volume 
df=[round(x, 3) for x in [1, 1/2, 1/3, 1/4, 1/10, 1/20]] #Dilution Factor
